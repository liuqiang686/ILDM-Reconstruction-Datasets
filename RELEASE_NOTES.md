...
Large files are provided as ordered `.part.###` assets. Download all assets and run `reassemble_dataset.py` from the repository to recover and verify the original MAT files. Exact SHA-256 checksums are provided in `dataset_manifest.json` and `SHA256SUMS`.
